...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    console.error(JSON.stringify({ url, options }));
...
router.get("/events", async (req, res) => {
  try {
    ...
    if (!id)
      return res
        .status(400)
        .json({ error: "Parameter 'id' is required", data: null });
...
router.get("/events_poll", async (req, res) => {
  try {
    ...
    if (!second_delay)
      return res
        .status(400)
        .json({ error: "second_delay is null!", data: null });
    if (!old_event)
      return res.status(400).json({ error: "old_event is null!", data: null });
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Invalid JSON in old_event", data: null });
    }
    ...
    while (oldDataString === newDataString) {
      ...
      if (count >= maxTry) {
        return res
          .status(408)
          .json({ error: `Max try = ${maxTry} polling reached!`, data: null });
      }
