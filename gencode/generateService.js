const { capitalize, toPascalCase } = require("./utils")
...
function generateService(sql) {
    ...
    if (!${columns[1]}) {
        throw Error("${capitalize(columns[1])} can't be null!")
    }
...
export async function getAll${capitalize(tableName)}({ pageIndex, pageSize }) {
...
${columns
    ...
    .map(
        c => `export async function get${capitalize(tableName)}By${toPascalCase(c)}({ ${c}, pageIndex, pageSize }) {
